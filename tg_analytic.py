...
# write data to csv
def statistics(user_id, command,ischat=False,countsend="None"):
    data = datetime.datetime.today().strftime("%Y-%m-%d")
    with open('data.csv', 'a', newline="") as fil:
        wr = csv.writer(fil, delimiter=';')
        wr.writerow([data, user_id, command,str(ischat),countsend])

def statistics_chat(chat_id, name_chat):
    data = datetime.datetime.today().strftime("%Y-%m-%d")
    with open('chats.csv', 'a', newline="") as fil:
        wr = csv.writer(fil, delimiter=';')
        wr.writerow([data, chat_id,name_chat])
...
def get_chat(id):
    ...
    count_message = len(df[df.id == id].command)
    r = dict(sended_counts=sended_counts,count_likes=count_likes,count_message=count_message)
    if not (str(a[a.chat_id == id].iloc[[-1]].name_chat.tolist()[0]) == "nan"):
        r["name"] = a[a.chat_id == id].iloc[[-1]].name_chat.tolist()[0]
    else:
        r["name"] = str(id)
    print(r)
    return r
def get_chats():
    a = pd.read_csv('chats.csv', delimiter=';', encoding='utf8')
    return a["chat_id"].unique().tolist()
...
def analysis(bid, user_id):
    ...
    numbers_messages = len(df["command"])
    x = np.array(df[df.countsend != "None"]["countsend"])
    print(df)
    print(df[df.countsend != "None"]["countsend"])
    y = x.astype(np.float)
    print(y)
    sended_counts = sum(y)

    count_chats = len(df[df.ischat == True]["id"].unique())
    ...
    if 'txt' in bid or 'тхт' in bid:
        ...
        return message_to_user + "\nnumbers_messages: " + str(numbers_messages) + "\nLike sended: " + str(sended_counts) + "\nCounted chats: " + str(count_chats)
def custom(app=None,foo=True):
    df = pd.read_csv('data.csv', delimiter=';', encoding='utf8')
    count_chats = len(df[df.ischat == True]["id"].unique())
    numbers_messages = len(df["command"])
    like_messages = len(df[df.command == "emoji like"]["command"])
    number_of_users = len(df['id'].unique())
    x = np.array(df[df.countsend != "None"]["countsend"])
    y = x.astype(np.float)
    sended_counts = sum(y)
    count_write_to_bot = len(df[df.ischat == False]["id"].unique())
    count_no_write_to_bot = len(df[df.ischat == True]["id"].unique())
    if foo:
        return f"""
    Groups {count_chats}
    Messages {numbers_messages}
    Wallets with/without bot {count_write_to_bot}/{count_no_write_to_bot}
    Likes {like_messages}
    Volume {sended_counts}
        """
    else:
        return dict(count_chats=count_chats,numbers_messages=numbers_messages,count_write_to_bot=count_write_to_bot,count_no_write_to_bot=count_no_write_to_bot,like_messages=like_messages,sended_counts=sended_counts)